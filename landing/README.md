...
The product is **Throughline**: it stitches scattered incident reports into one story per failure,
prices each story, and drafts the eval case for the ones that cost the most.

Five landing pages, one per visual direction. Every page carries the same facts from the same
...
| 03 | [03-swiss.html](03-swiss.html) | **Swiss** — visible 12-column grid, very large tight type, exactly one colour. | light | you want it to look expensive |
...
| 05 | [05-signal.html](05-signal.html) | **Signal** — dark, one luminous accent, the argument staged as the centrepiece. | dark | you are presenting live |
...
```sh
for spec in 01-ledger:light 02-terminal:dark 03-swiss:light 04-instrument:light 05-signal:dark; do
  n="${spec%%:*}"; s="${spec##*:}"
